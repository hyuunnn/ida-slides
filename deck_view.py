"""Platform-neutral deck rendering: the marp/slidev pipeline and the shared
in-page JS, factored out of the per-platform webview embeddings.

`DeckViewBase` owns everything that is NOT the native webview: engine
detection, the marp `-w` watcher and slidev dev-server pipelines, embed
preprocessing, status text, and the @token message routing. The native
webview itself (creation, navigation, JS eval, focus) is delegated to a
small hook surface implemented per platform:

    webkit_view.DeckWebKitView      macOS  — WKWebView via PyObjC
    webview2_view.DeckWebView2View  Windows — WebView2 via ctypes COM

Subclass contract (all hooks may assume the Qt UI thread):
    _native_attach()            create the webview inside self._container.
                                May complete asynchronously; on success set
                                self._web (any truthy handle) and call
                                self._attach_done(). Raise, or call
                                self._attach_failed(msg), on failure.
    _native_teardown()          drop the webview; never raises.
    _native_load_url(url)       navigate to an http(s)/file URL string.
    _native_load_file(path)     display a local html file.
    _native_eval_js(js)         fire-and-forget JS in the page.
    _native_eval_js_result(js, cb)
                                eval JS; deliver cb(str | None) DEFERRED via
                                QTimer.singleShot(0, ...) — never from inside
...
# them to Python. Marp output is a static DOM, but Slidev is a Vue SPA that
# mounts slides dynamically, so linkification re-runs through a
# MutationObserver (disconnected during our own rewrites to avoid feedback
# loops). The `post` shim is the platform bridge: WebView2 exposes
# window.chrome.webview, WKWebView exposes window.webkit.messageHandlers.
# WebView2 injects this at document-created (before <body> exists), WKWebView
...
(function () {
    ...
    document.addEventListener('mouseover', function (ev) {
        ...
        hoverTimer = setTimeout(function () {
            ...
            a.__idaReq = id;
            post({type: 'preview', name: name, line: line, id: id, key: key});
        }, 250);
    }, true);
    ...
    document.addEventListener('click', function (ev) {
        ...
        if (xref) {
            ...
            ev.stopPropagation();
            hideTip();
            post({type: 'jump', name: xref.getAttribute('data-ida-name'),
...
def _node_version_key(path: str) -> tuple[int, ...]:
    ...
    `...\\nvm\\v22.1.0\\...`). Paths without one sort lowest."""
    ...
    if not path:
        ...
        for pattern in _NODE_BIN_GLOBS:
            for suffix in _TOOL_SUFFIXES:
                hits.extend(glob.glob(os.path.join(pattern, name + suffix)))
            if hits:
...
# real JS entry points behind the npm launcher shims, per tool
_NODE_CLI_ENTRIES = {
    "marp": os.path.join("node_modules", "@marp-team", "marp-cli", "marp-cli.js"),
    "slidev": os.path.join("node_modules", "@slidev", "cli", "bin", "slidev.mjs"),
}
...
def _spawn_spec(tool_path: str) -> tuple[str, list[str]]:
    ...
    POSIX runs the tool script directly. Windows npm installs launcher
...
def dispatch_page_message(owner: "DeckViewBase", body: dict) -> None:
    ...
    Shared by both platform bridges (WKScriptMessageHandler and WebView2's
    WebMessageReceived). Callers invoke this from inside a native callback
    frame, so ALL IDA work is deferred via QTimer.singleShot(0). Never
    raises — an exception escaping a native callback can abort IDA.
    """
    try:
        ...
        logger.exception("page message dispatch failed")


class DeckViewBase(QWidget):
    """Renders a Marp/Slidev deck with a platform-native webview.
    ...
    # subclass overrides: status text when the native attach breaks
    _ATTACH_FAIL_MSG = "webview attach failed — see Output window"
    # short renderer name for the toolbar status ("Marp/<label>")
    renderer_label = "webview"
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self._web = None                         # native handle; set on attach
    ...
    def _attach_webview(self) -> None:
        ...
        try:
            ...
            logger.exception("failed to attach native webview")
            self._attach_failed(self._ATTACH_FAIL_MSG)

    def _attach_done(self) -> None:
        """Called by the subclass once self._web is live (possibly async)."""
        if self._path:
    ...
    # ------------------------------------------------------------------
    # subclass hook surface (see module docstring)
    # ------------------------------------------------------------------
    ...
    def load(self, path: str, restore_hash: str | None = None) -> bool:
        ...
        if self._web is None:
            return True  # _attach_done picks it up
    ...
    def reload(self) -> None:
        ...
        def _captured(result: str | None) -> None:
            # delivered via singleShot (hook contract), so IDA work — the
            # embed decompile inside load() — is safe here
            try:
                if self._path != path:
                    return  # deck switched since reload started
                self.load(path, restore_hash=result or None)
            except Exception:
                logger.exception("hash-capture completion failed")

        try:
            self._native_eval_js_result("window.location.hash", _captured)
        except Exception:
    ...
    @staticmethod
    def _configure_tool_proc(proc: QProcess, tool: str, args: list[str]) -> None:
        """Point `proc` at a node CLI tool (see _spawn_spec)."""
        program, pre = _spawn_spec(tool)
        proc.setProgram(program)
        proc.setArguments(pre + args)
        proc.setProcessEnvironment(DeckViewBase._tool_env(tool))
    ...
    def _run_slidev(self, md_path: str, text: str | None = None) -> None:
        ...
        if (
            ...
            self._native_load_url(f"http://localhost:{self._slidev_port}/")
            return
        ...
        proc = QProcess(self)
        self._configure_tool_proc(
            proc, slidev, [prepared, "--port", str(port), "--force"]
        )
    ...
    def _run_marp(
        ...
        if fresh:
            ...
            proc = QProcess(self)
            self._configure_tool_proc(
                proc, marp, ["-w", prepared, "-o", out, "--html", "--no-stdin"]
            )
            # marp blocks reading stdin when it is a pipe; --no-stdin plus a
            # null stdin covers both marp's TTY probe and the read itself
            proc.setStandardInputFile(QProcess.nullDevice())
    ...
    def _finish_render(self, out: str, restore_hash: str | None = None) -> None:
        ...
        # so no earlier in-flight load's completion can consume it
        self._pending_hash = restore_hash
        self._native_load_file(out)
    ...
    # ------------------------------------------------------------------
    # page callbacks (invoked deferred, on the UI thread)
    # ------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# platform dispatch (consumed by presenter_form)
# ---------------------------------------------------------------------------
def availability_error() -> str | None:
    """None when this platform can render decks, else a user-facing reason."""
    if sys.platform == "darwin":
        ...
        if webkit_view.webkit_available():
            return None
        return (
            "ida-slides: rendering requires PyObjC (native WKWebView).\n\n"
            "Install with: pip install --user pyobjc-framework-WebKit"
        )
    if sys.platform == "win32":
        import webview2_view

        return webview2_view.availability_error()
    return (
        "ida-slides: rendering is supported on macOS (WKWebView) and "
        "Windows (WebView2) only."
    )


def create_renderer() -> DeckViewBase:
    """The platform renderer; call only after availability_error() is None."""
    if sys.platform == "win32":
        import webview2_view

        return webview2_view.DeckWebView2View()
    import webkit_view

    return webkit_view.DeckWebKitView()
